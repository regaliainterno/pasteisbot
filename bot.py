...
def gerar_dados_relatorio_diario(data_filtro):
    ...
    titulo_relatorio = f"📊 *Fechamento do Dia: {data_filtro.strftime('%d/%m/%Y')}*"
    resumo_financeiro = (f"💰 *RESUMO FINANCEIRO*\n"
                         f"  - Faturamento Bruto: *R$ {df_vendas_dia['total_venda'].sum():.2f}*\n"
                         f"  - Lucro (Margem das Vendas): *R$ {df_vendas_dia['lucro_venda'].sum():.2f}*")
    ...
    return {
        ...
        "faturamento_bruto": float(df_vendas_dia['total_venda'].sum()),
        "lucro_margem": float(df_vendas_dia['lucro_venda'].sum()),
        "custo_investimento": float(df_estoque_dia['quantidade_inicial'].sum() * PRECO_FIXO_CUSTO),
        "custo_consumo": float(df_consumo_dia['custo_total'].sum()),
        "resultado_final": float(df_vendas_dia['lucro_venda'].sum() - df_consumo_dia['custo_total'].sum()),
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text = (
        f"Olá, {user_name}! Bem-vindo ao seu assistente de gestão v12!\n\n"
...
async def ver_estoque_atual(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        # Pré-calcula os totais de vendas e consumo para hoje
        ...
        for index, row in estoque_hoje.iterrows():
            ...
            # Busca os totais de forma segura, retornando 0 se não houver registros
...
# ... (outras funções de comando como registrar_usuario, definir_estoque, registrar_venda, consumo_pessoal, gerar_grafico, etc. continuam aqui)
